...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
        ...
        poseIncrement.apply(source_point, temp_point);

        for (int i = 0; i < 3; i++) {
            residuals[i] = (temp_point[i] - target_point[i]) * T(m_weight) * T(LAMBDA);
        }
		return true;
	}
...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
        ...
        // T dot_prod = target_norm.dot(temp_point - target_point);
        // residuals[0] = T(LAMBDA) * dot_prod;

        for (int i = 0; i < 3; i++) {
            residuals[0] = (temp_point[i] - target_point[i]) * target_norm[i];
        }

        residuals[0] *= T(LAMBDA);
	...
	void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			if (match.idx >= 0) {
                ...
                // auto angle = std::acos(sourceNormal.dot(targetNormal) );
				if(angle > M_PI / 3.0)
				    match.idx = -1;
			}
		}
...
private:
	...
	Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
		...
		VectorXf x(6);
        x = x = A.bdcSvd(ComputeThinU | ComputeThinV).solve(b);
