...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
        ...
        double dist_to_surface = sqrt(pow((_x[0] - m_center[0]), 2) + pow((_x[1] - m_center[1]),2) + pow((_x[2] - m_center[2]),2)) - m_radius;
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
        ...
        double a = sqrt(pow((_x[0] - m_center[0]), 2) + pow((_x[2] - m_center[2]), 2)) - m_radius;
		double dist_to_surface = sqrt(pow(a,2) + pow((_x[1] - m_center[1]),2)) - m_a;
        return dist_to_surface;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
        ...
        {
            // First term of f(x): multiply the norm of p_i - x with alpha coefficents
            result += EvalBasis((m_funcSamp.m_pos[i] - _x).norm()) * m_coefficents[i];
        }

        // Second term of f(x) - linear part: multiply x with b coefficents
        result += _x[0] *  m_coefficents[m_numCenters + 0];
        result += _x[1] *  m_coefficents[m_numCenters + 1];
        result += _x[2] *  m_coefficents[m_numCenters + 2];

        // Final term of f(x) - constant part: add coefficent d
	...
	void BuildSystem()
	{
        ...
        for (int i = 0; i < 2 * m_numCenters; i++) {
            for (int j = 0; j < m_numCenters + 4; j++){
                ...
                if (j < m_numCenters){
                    // Phi values for on / off surface points
                    A(i,j) = phi(i,j);
                }
                else if ((j >= m_numCenters) && (j < m_numCenters + 3)){
                    // x,y,z values for on / off surface points
                    A(i,j) = m_funcSamp.m_pos[i][j-m_numCenters];
                }
                else{
                    A(i,j) = 1;
                }
