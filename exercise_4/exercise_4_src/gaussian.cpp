...
struct GaussianCostFunction
{
	...
	bool operator()(const T* const mu, const T* const sigma, T* residual) const
	{
        ...
        auto constant_val = T(1.0 / sqrt(2.0 * M_PI));
        auto exponent_val  = T(exp (- pow(point.x-*mu,2) / (T(2) * pow(*sigma,2))));
        residual[0] = constant_val / sqrt(pow(*sigma,2));
        residual[0] = residual[0] * exponent_val;
...
int main(int argc, char** argv)
{
	...
	{
	    auto cost_func = new ceres::AutoDiffCostFunction<GaussianCostFunction, 1, 1, 1>(new GaussianCostFunction(point));
		problem.AddResidualBlock(cost_func,	nullptr, &mu, &sigma);
	}

	ceres::Solver::Options options;
	options.max_num_iterations = 10; // Setting this to 15, converges
