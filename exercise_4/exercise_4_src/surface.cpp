...
{
    SurfaceCostFunction(const Point3D& _point): point(_point){}

    template<typename T>
    bool operator()(const T* const a, const T* const b, const T* const c, T* residual) const{

        auto cost = ((T(pow(point.x,2)) / *a) - (T(pow(point.y,2)) / *b)) / *c;
        residual[0] = point.z - cost;
        return true;
    }
private:
    const Point3D point;
...
int main(int argc, char** argv)
{
    ...
    for(auto& point : points){
        auto cost_func = new ceres::AutoDiffCostFunction<SurfaceCostFunction,1,1,1,1>(new SurfaceCostFunction(point));
